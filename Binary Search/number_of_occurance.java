...
public class number_of_occurance
{
    ...
    public static void main(String[] args)
    {
        ...
        // BINARY SEARCH
        int target,x,y = 0;
        System.out.println("Enter the element of which you are finding occurance.");
        target = sc.nextInt();
        x = lower(arr,n,target);
        y = upper(arr,n,target);
        if(x == n || arr[x] != target)
        {
            System.out.println(target+" is not in your array.");
        }
        else
        {
            System.out.println(target+" occured "+((y-x)+1)+" times.");
        }
        // PRINTING
        for(int i = 0; i < n; i++)
        {
